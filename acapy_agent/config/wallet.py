...
async def wallet_config(
    ...
    if provision:
        ...
        LOGGER.info(
            "Profile backend: %s, Profile name: %s", profile.backend, profile.name
        )
    ...
    if public_did_info:
        ...
        if wallet_seed and seed_to_did(wallet_seed) != public_did:
            if context.settings.get("wallet.replace_public_did"):
                ...
                LOGGER.info(
                    ...
                    f"Verkey: {replace_did_info.verkey}"
                )
            else:
        ...
        if wallet_local_did:
            ...
            if provision:
                LOGGER.info(
                    f"Created new local DID: {local_did}, Verkey: {local_did_info.verkey}"
                )
        else:
            ...
            if provision:
                LOGGER.info(
                    f"Created new public DID: {public_did}, "
                    f"Verkey: {public_did_info.verkey}"
                )
            # wait until ledger config to set public DID endpoint - wallet goes first
